...
public sealed class CotisationOutboxProcessor : BackgroundService
{
    private readonly IServiceScopeFactory _serviceScopeFactory;
    private readonly RabbitMqOptions _options;
    private readonly ILogger<CotisationOutboxProcessor> _logger;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(exception, "Failed to process the cotisation outbox.");
            }
    ...
    private async Task ProcessBatchAsync(CancellationToken cancellationToken)
    {
        ...
        var dbContext = scope.ServiceProvider.GetRequiredService<CotisationDbContext>();
        var publisher = scope.ServiceProvider.GetRequiredService<CotisationOutboxPublisher>();
        ...
        foreach (var candidateId in candidateIds)
        {
            ...
            catch (Exception exception)
            {
                _logger.LogError(
                    exception,
                    "Failed to publish cotisation outbox message {OutboxMessageId}.",
                    candidateId);
